...
wind <- array(0, dim = c(speed.bins, dir.bins)) #preallocation (for memory, required)
for (t in 1:nrow(hfp)) {
...
wind.long <- array(NA, dim = dir.bins*speed.bins) #preallocation, NA used because EVERy cell will be replaced 
speeds <- c(rep("0-1",dir.bins), rep("1-2",dir.bins), rep("2-3",dir.bins), rep("3-4",dir.bins), rep("4-5",dir.bins), rep("6-7",dir.bins)) # be sure to fill in as many as the wind bins in "wind" allocation
...
ggplot(rose, aes(fill = fct_rev(speeds), x = directions, y = wind.long)) +
  ...
  scale_fill_brewer("Speed (m/s)", palette = "Oranges") +
  coord_polar(theta = "x", start = 0) +
  scale_x_continuous(breaks = seq(0, 360, 45)) +
  theme_linedraw() +
